...
$logPath = Join-Path $LogRoot "claude-advisor-$safeStamp.log"
$runner = Join-Path $PSScriptRoot 'Invoke-ClaudeAdvisorTask.ps1'
...
try {
    Write-AiwgEvent -WorkgroupRoot $workgroupRoot -Agent 'Orchestrator' -Type 'claude_advisor_scheduled_started' -Status 'read_only=True'
    $output = & $runner `
    ...
    if ($exitCode -ne 0) {
        Write-AiwgEvent -WorkgroupRoot $workgroupRoot -Agent 'Orchestrator' -Type 'claude_advisor_scheduled_failed' -Path $logPath -Status "exit_$exitCode"
        exit $exitCode
    }

    Write-AiwgEvent -WorkgroupRoot $workgroupRoot -Agent 'Orchestrator' -Type 'claude_advisor_scheduled_finished' -Path $logPath -Status 'ok'
    exit 0
} catch {
    Write-LogLine "error=$($_.Exception.Message)"
    Write-AiwgEvent -WorkgroupRoot $workgroupRoot -Agent 'Orchestrator' -Type 'claude_advisor_scheduled_failed' -Path $logPath -Status $_.Exception.Message
    throw
}
