...
New-Item -ItemType Directory -Force -Path $claudeInbox, $codeXInbox, $done | Out-Null

$readyMessages = New-Object System.Collections.ArrayList
foreach ($candidate in (Get-ChildItem -LiteralPath $claudeInbox -Filter '*.md' -File -ErrorAction SilentlyContinue)) {
...
if ($readyMessages.Count -gt 0) {
    throw "Claude-Code inbox already contains ready messages; refusing controlled smoke test. Files: $($readyMessages -join ', ')"
}

$startedAt = Get-Date
$timestamp = New-FileTimestamp
$messageId = "CW0-msg-$timestamp"
$task = "CW0-claude-watcher-$timestamp"
$messageName = "${timestamp}_from-CodeX_to-Claude-Code_type-instruction_task-$task.md"
$messagePath = Join-Path $claudeInbox $messageName
...
$watchJob = Start-Job -ScriptBlock {
    ...
    Set-Location $Root
    & 'scripts/ai-workgroup/watch-inbox.ps1' -Agents Claude-Code -SkipInitialScan -AllowExternalAgents -PollSeconds 60 -StopAfterSeconds $StopAfter -Json
...
Run the CW0 Claude-Code watcher smoke test.

Acceptance:

- This task is dispatched by watch-inbox.ps1, not by manually calling claude-headless-runner.ps1.
- Runner policy allows this read-only Claude-Code task.
- Claude-Code writes one report to docs/ai-workgroup/inbox/CodeX/.
...
"@
    ...
    if ($null -eq $report) {
        throw "Expected Claude-Code report with reply_to $messageId was not found within $TimeoutSeconds seconds. Watch output: $($watchOutput -join "`n")"
    }
