...
MESSAGE_ID = "D5313-msg-gate-config-error"
TASK_ID = "D5313-gate-config-error"


def build_d5313_config(tmp_path: Path, *, mode: str = "sandbox_plan") -> dict[str, Any]:
    ...
    config["real_adapter_sandbox"] = {
        ...
        "probe_command": [sys.executable, "-c", "print('d5313-probe-ok')"],
    }
...
def write_d5313_message(project_root: Path) -> Path:
    path = (
        project_root
        / "docs"
        / "ai-workgroup"
        / "inbox"
        / "OpenCode"
        / f"2026-06-17T020000_from-CodeX_to-OpenCode_type-instruction_task-{TASK_ID}.md"
    )
    ...
    path.write_text(
        "\n".join(
            [
                ...
                "---",
                "",
                "# D5.3.13 adapter readiness gate config error fixture",
                "",
            ]
        ),
        encoding="utf-8",
...
def create_approved_preflight(tmp_path: Path, *, mode: str = "sandbox_plan") -> tuple[dict[str, Any], Path, Path, str]:
    config = build_d5313_config(tmp_path, mode=mode)
    db_path = init_database(config=config, project_root=tmp_path)
    write_d5313_message(tmp_path)
    ...
    approval = approve_preflight(
        ...
        ttl_minutes=60,
        reason="D5.3.13 adapter readiness gate approval",
    )
...
def prepare_plan_and_probe_chain(tmp_path: Path) -> tuple[dict[str, Any], Path, str, Path, Path]:
    ...
    probe_config = build_d5313_config(tmp_path, mode="sandbox_probe")
...
def make_current_readiness_config_malformed(config: dict[str, Any]) -> None:
    config["adapter_binary_readiness"]["version_probe_enabled"] = "false"


def test_gate_returns_structured_block_when_current_readiness_config_is_malformed(tmp_path: Path) -> None:
    config = build_d5313_config(tmp_path)
    db_path = init_database(config=config, project_root=tmp_path)
    ...
    assert any("adapter_binary_readiness.version_probe_enabled" in error for error in result.payload["errors"])


def test_resume_preflight_records_standard_block_when_current_readiness_config_is_malformed(tmp_path: Path) -> None:
...
def test_resume_preflight_preserves_config_error_from_blocked_latest_readiness_report(tmp_path: Path) -> None:
    config, db_path, _manifest_path, approval_id = create_approved_preflight(tmp_path)
    make_current_readiness_config_malformed(config)
